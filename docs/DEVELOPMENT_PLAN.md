...
| 积分系统 | 积分累计、积分消费、积分排行 | 🟡 审核发放、累计积分、学生排行与积分明细已接通，积分消费与抽奖待开发 |
| 抽奖系统 | 积分抽奖、奖品管理 | ⬜ 待开发 |
| 排行榜 | 学生积分排行、任务完成排行 | 🟡 学生周榜/月榜/总榜真实数据、快照刷新与后台排行榜查看已完成，班级榜待开发 |
| 配置系统 | 后台参数配置、系统设置 | ✅ 后台系统配置 CRUD、默认配置初始化、操作日志与小程序侧配置回退已完成 |
| 后台管理 | Web 管理端、管理员权限、运营与业务数据管理 | 🟡 Vue 3 后台、登录校验、运营概览、配置、项目、用户、班级、任务、提交、排行、公告、积分明细、日志共 12 个页面已完成，待部署验收 |
...
├── admin-web/                  # 后台管理网站
│   ├── src/api/                # 后台云函数封装
│   ├── src/pages/              # 运营概览、配置、项目、用户、班级、任务、提交、排行、公告、积分明细、日志
│   ├── src/router/             # Hash 路由
│   └── src/stores/             # 登录态与应用状态
│
...
| submit-task | 提交任务 | ✅ 已完成 |
| get-submissions | 获取提交记录 | ✅ 已完成 |
...
const TEACHER_TABBAR = [
  ...
  {
    value: '/pages/teacher/task-manage/task-manage',
    icon: 'task',
    label: '任务'
  },
  {
...
```json
{
  "pages": [
    ...
    "pages/student/task-manage/submission-edit/submission-edit",
    "pages/student/task-manage/submission-records/submission-records",
...
项目已实现 `config/project.js` 配置服务，当前支持：

1. **默认项目配置** - 内置编程、无人机、机器人三类默认项目
2. **默认系统配置** - 内置积分、抽奖、班级、任务相关默认参数
3. **本地缓存** - 配置数据支持本地缓存，减少重复读取
4. **服务层与云函数已接入** - `get-projects` 已提供云函数实现，`get-config` 仅保留前端调用入口，未落地时自动回退默认配置
