...
```
  ...
  > 2026-09-19 24 小时复盘：错误、超时、限流均为 0；但低流量时出现连续三个 5 分钟活跃窗口平均耗时 696ms、697ms、651ms，每个窗口各 1 次调用。结束后的覆盖窗口内，三个活跃窗口非零平均耗时为 458.56ms、最大 732ms，仍高于 400ms 目标。因此稳定性通过，公告耗时验收未通过；下一轮应优先排查冷启动与每次请求仍执行的已读记录查询，并在至少三个有代表性的活跃小时重新验收。
...
- [x] **步骤 5：观察线上 24 小时监控**
  ...
  > 监控窗口：2026-09-18 17:09 至 2026-09-19 17:09（Asia/Hong_Kong）。定时任务每次只查询最近约 4 小时的 CloudBase 指标，收尾查询于 18:40 执行并覆盖窗口结束时刻。所列耗时均为 5 分钟聚合平均值，不是单次请求分位数，也不能替代页面 P75/P95。

  | 观察项 | 24 小时结果 | 结论 |
  |---|---|---|
  | 错误 / 超时 / 限流 | 各轮 `FunctionError`、`FunctionTimeout`、`FunctionThrottle` 均为 0 | 稳定性通过 |
  | 学生聚合 | 部署后活跃窗口平均 49.39ms、最大 189ms；后续定时窗口无调用 | 未发现超过 500ms，但活跃样本不足 |
  | 教师聚合 | 部署后活跃窗口平均 80.28ms、最大 335.67ms；后续定时窗口无调用 | 未发现超过 500ms，但活跃样本不足 |
  | 审核聚合 | 部署后活跃窗口平均 105.62ms、最大 368ms；后续定时窗口无调用 | 未发现超过 600ms，但活跃样本不足 |
  | 公告接口 | 初始一小时平均 225.28ms；随后连续三个低流量窗口为 696ms、697ms、651ms；收尾覆盖窗口的活跃样本非零平均 458.56ms、最大 732ms | 超过 400ms 目标，未通过 |
  | 其他重点接口 | 收尾覆盖窗口的活跃样本非零平均：任务 259.44ms、提交 344.33ms、排行榜 240ms、班级 233.88ms、入班申请 302.11ms | 未发现平台错误，作为后续基线保留 |
...
2026-09-19 24 小时监控复盘：2026-09-18 17:09 至 2026-09-19 17:09 的定时观察已结束，各轮环境级错误、超时、限流均为 0。学生、教师、审核聚合函数在后续窗口没有足够真实调用，不能据此宣称达到连续活跃时段目标；公告函数曾连续三个 5 分钟活跃窗口达到 696ms、697ms、651ms，且收尾覆盖窗口的活跃样本非零平均为 458.56ms、最大 732ms，超过 400ms 目标。所有 CloudBase `FunctionDuration` 数值均为 5 分钟聚合平均值；本轮没有同设备 20 次页面计时数据，因此 P75/P95 仍未测量。
...
| 公告平均耗时 | 671～794ms | 初始一小时 225.28ms；后续连续窗口 696/697/651ms；收尾活跃样本非零平均 458.56ms | ≤400ms | 未通过，需排查冷启动和已读查询 |
| 学生聚合接口平均耗时 | 无 | 49.39ms（部署后活跃窗口）；后续窗口无调用 | ≤500ms | 未发现超标，活跃样本不足 |
| 教师聚合接口平均耗时 | 无 | 80.28ms（部署后活跃窗口）；后续窗口无调用 | ≤500ms | 未发现超标，活跃样本不足 |
| 审核聚合接口平均耗时 | 无 | 105.62ms（部署后活跃窗口）；后续窗口无调用 | ≤600ms | 未发现超标，活跃样本不足 |
| 主包体积 | 待开发者工具确认 | 769,878 bytes | ≤1.5MiB | 通过 |
| 学生首页 P75 | 待真机测量 | 未实施 | ≤1.5s | 未验收 |
| 教师首页 P75 | 待真机测量 | 未实施 | ≤1.8s | 未验收 |
| 审核中心 P75 | 待真机测量 | 未实施 | ≤1.5s | 未验收 |
| 云函数错误/超时/限流 | 0/0/0 | 24 小时定时观察各轮均为 0/0/0 | 0/0/0 | 通过 |
