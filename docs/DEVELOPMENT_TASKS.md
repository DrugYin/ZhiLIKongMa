...
| 分包静态体积 | 主包约 381,207 bytes；学生 140,301 bytes；教师 121,536 bytes；公共 7,886 bytes | 开发者工具登录后确认最终上传包体 |
| 线上变更 | 新增 3 个聚合云函数；新增 8 个复合索引并回读确认；未覆盖现有线上函数 | 体验版验证后再部署兼容调整与公告优化 |
| 性能指标 | 尚无部署后 P75/平均耗时 | 同设备、同网络各测试 20 次并观察 24 小时监控 |

本轮已完成本地代码、静态调用路径和第一阶段灰度部署：三个新增聚合云函数状态为可用，缺少微信身份时返回业务 `401`；八个复合索引已在线创建并回读。现有线上函数尚未覆盖，因此正式版客户端仍走原有接口。微信开发者工具 CLI 此前因账号需要重新登录（错误码 10）中止；真实角色场景、体验版上传、现有函数更新和 24 小时监控仍待完成。详细步骤和回滚方案见 `docs/MINIPROGRAM_PERFORMANCE_OPTIMIZATION_TASK_PLAN.md`。
