...
| 9.2.2 | 云函数性能优化 | P1 | 🟡 3 个聚合接口已灰度部署，公告缓存与现有函数待体验版验证后部署 |
| 9.2.3 | 前端加载优化 | P1 | 🟡 首页请求合并、分包和长列表限制已完成，待真机验收 |
| 9.2.4 | 图片资源优化 | P2 | ⬜ |
...
| 线上变更 | 新增 3 个聚合云函数；新增 8 个复合索引并回读确认；未覆盖现有线上函数 | 体验版验证后再部署兼容调整与公告优化 |
| 性能指标 | 尚无部署后 P75/平均耗时 | 同设备、同网络各测试 20 次并观察 24 小时监控 |

本轮已完成本地代码、静态调用路径和第一阶段灰度部署：三个新增聚合云函数状态为可用，缺少微信身份时返回业务 `401`；八个复合索引已在线创建并回读。现有线上函数尚未覆盖，因此正式版客户端仍走原有接口。开发者工具预览编译和代码上传已成功并生成二维码，主包为 769,878 bytes；真实角色扫码验收、现有函数更新和 24 小时监控仍待完成。详细步骤和回滚方案见 `docs/MINIPROGRAM_PERFORMANCE_OPTIMIZATION_TASK_PLAN.md`。
