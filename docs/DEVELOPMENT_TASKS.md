...
| 序号 | 任务 | 优先级 | 状态 |
|------|------|--------|------|
| 9.2.1 | 数据库查询优化 | P0 | 🟡 已精简字段并生成 8 个复合索引配置，待线上创建与回读 |
| 9.2.2 | 云函数性能优化 | P1 | 🟡 聚合接口、公告缓存与分页已完成，待部署监控 |
...
| 项目 | 本地结果 | 尚待完成 |
|------|----------|----------|
| 自动化回归 | `tests/*.test.js` 共 16 项，16/16 通过 | 部署后核心业务真机回归 |
...
| 线上变更 | 未执行 | 部署 3 个聚合云函数及相关函数，创建 8 个复合索引 |
| 性能指标 | 尚无部署后 P75/平均耗时 | 同设备、同网络各测试 20 次并观察 24 小时监控 |

本轮只完成并验证了本地代码和静态调用路径。微信开发者工具 CLI 构建因账号需要重新登录（错误码 10）中止；线上部署、索引创建、真实业务数据抽样、体验版上传及 24 小时监控均未擅自执行。详细步骤和回滚方案见 `docs/MINIPROGRAM_PERFORMANCE_OPTIMIZATION_TASK_PLAN.md`。
